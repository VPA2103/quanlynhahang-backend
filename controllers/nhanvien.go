...
func UpdateOwnProfile(c *gin.Context) {
	userID := c.GetUint("user_id") // Lấy ID từ JWT
	var nv models.NhanVien

	// ✅ Tìm nhân viên hiện tại
	if err := config.DB.Preload("AnhNhanVien").First(&nv, userID).Error; err != nil {
		c.JSON(http.StatusNotFound, gin.H{"error": "Không tìm thấy nhân viên"})
		return
	}
	...
	var updatedData struct {
		HoTen   string `form:"ho_ten"`
		SDT     string `form:"sdt"`
		DiaChi  string `form:"dia_chi"`
		Email   string `form:"email"`
		MatKhau string `form:"mat_khau"`
	}
	if err := c.ShouldBind(&updatedData); err != nil {
	...
	// ✅ Lưu thay đổi
	if err := config.DB.Save(&nv).Error; err != nil {
		c.JSON(http.StatusInternalServerError, gin.H{"error": "Không thể cập nhật thông tin: " + err.Error()})
		return
	}
